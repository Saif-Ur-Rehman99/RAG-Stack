...
def _get_database():
    return MongoDatabaseConnector().get_database(settings.DATABASE_NAME)
...
class NoSQLBaseDocument(BaseModel, Generic[T], ABC):
    ...
    @classmethod
    def find_sorted(cls: Type[T], sort: list, **filter_options) -> T | None:
        collection = _get_database()[cls.get_collection_name()]
        try:
            instance = collection.find_one(filter_options, sort=sort)
            if instance:
                return cls.from_mongo(instance)
            return None
        except errors.OperationFailure:
    ...
    def get_collection_name(cls: Type[T]) -> str:
        if not hasattr(cls, "Settings") or not hasattr(cls.Settings, "name"):
            raise ValueError(
                f"Document class {cls.__name__} must define a nested 'Settings' class with a 'name' attribute."
            )
        return cls.Settings.name